...
let Room = require('../models/popeyewalletDat')

let User = require('../models/messageData')
let Bet = require('../models/popeyewalletDat');
...
module.exports = function () {
    ...
    router.post('/add_walet', function (req, res) {
        let RoomData = new Bet(req.body);
        RoomData.save()
            .then(Room => {
                var data = {
    ...
    router.get('/get_wallet_Balance', function (req, res) {
        var found=false;
        Bet.find(function (err, data) {
            if (!err) {
                data.forEach(element => {
                    if (element.WalletAddress == req.body.WalletAddress) {
